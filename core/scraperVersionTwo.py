    ...
    if 'download' in request.POST:
        if download_type == "INCOME_STATEMENT": 
            ...
            return response
                ...
                res = AsyncResult(task_id).get()
                df = pd.read_json(res)
                df.to_excel('operating_performance.xls', index=False)
                with open("operating_performance.xls", 'rb') as file:
                        response = HttpResponse(file, content_type='application/vnd.ms-excel')
                        response['Content-Disposition'] = 'attachment; filename=stockData.xls'   
                        return render(request,response, "../templates/loadScreen.html")
                ...
                res = AsyncResult(task_id).get()
                df = pd.read_json(res)
                df.to_excel('valuation_growth.xls', index=False)
                ...
                res = AsyncResult(task_id).get()
                df = pd.read_json(res)
                df.to_excel('valuation_financial_health.xls', index=False)
                ...
                res = AsyncResult(task_id).get()
                df = pd.read_json(res)
                df.to_excel('valuation_operating_efficiency.xls', index=False)
